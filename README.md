...
Ως αγαπημένο προϊόν ορίζεται αυτό που έχει εμφανιστεί τις περισσότερες φορές στο καλάθι του, ανεξαρτήτως ποσότητας. Αυτό γίνεται για να μην παίρνουν προτεραιότητα τα προϊόντα από τα οποία αγοράζουμε μεγάλη ποσότητα ούτως ή άλλως (πχ αν αγοράσει μήλα ή ντομάτες, τότε προφανώς η ποσότητα θα είναι πάντα μεγαλύτερη από αυτή που θα ήταν αν αγόραζε μπλούζες). Το προϊόν που εμφανίστηκε τις περισσότερες φορές στο καλάθι το υπολογίζει η μέθοδος `getDiscount`.

Τέλος, στο αρχείο `loyal_discounts.txt` αποθηκεύεται η πληροφορία του αν ένας χρήστης έχει εξαργυρώσει την έκπτωση του 40% στις 5 παραγγελίες του. Αυτό το αρχείο διαβάζεται στην συνάρτηση loadUsers() του Eshop προκειμένουν να ενημερώσει ανάλογα το πεδίο του Customer.
  
Στα samples έχουμε παραθέσει ένα αρχείο sample_customer.DISCOUNT.txt, όπου περιέχει μια αλληλεπίδραση στην οποία ο χρήστης δέχεται discounts (με τα original order histories που μας δόθηκαν).
...
# compilation
Στο φάκελο υπάρχει makefile. Απλά τρέξτε:
```
make
``` 
Για να παράξετε το εκτελέσιμο `oop24`.